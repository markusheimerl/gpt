...
// Tokenize text corpus to binary token file
void tokenize_corpus(const char* text_path, const char* bin_path) {
    FILE* text_file = fopen(text_path, "r");
    if (!text_file) { fprintf(stderr, "Cannot open %s\n", text_path); exit(1); }
...
void generate_text(GPT* gpt, float temperature, unsigned short* d_input_tokens, const char* prompt, int gen_len) {
    ...
    int fd = mkstemp(temp);
    FILE* tf = fdopen(fd, "w");
    for (int i = 0; i < gen_len && i < gpt->seq_len; i++) fprintf(tf, "%hu ", h_tokens[i]);
    fclose(tf);
...
int main(int argc, char* argv[]) {
    ...
    char bin_path[512];
    snprintf(bin_path, sizeof(bin_path), "%s.bin", corpus_path);
    if (get_file_size(bin_path) == 0) tokenize_corpus(corpus_path, bin_path);
    else printf("Using cached tokenization: %s\n", bin_path);

    // Create shuffled indices for random sampling without replacement
    size_t total_sequences = (get_file_size(bin_path) - 2) / (2 * seq_len);
    ...
    for (size_t chunk_idx = start_chunk; chunk_idx < (total_sequences / sequences_per_chunk); chunk_idx++) {
        ...
        sample_sequences(bin_path, &shuffled_indices[chunk_idx * sequences_per_chunk], seq_len, input_tokens, target_tokens, sequences_per_chunk);
        ...
        for (int batch = batch_start; batch < (int)(sequences_per_chunk / batch_size); batch++) {
            ...
            if ((batch + 1) % accum_steps == 0) {
                ...
                printf("Chunk [%zu/%zu], Batch [%d/%d], Loss: %.6f, LR: %.7f, dt: %.2fms, tok/s: %.0f, bpb: %.4f, ETA: %.1fh\n",
                    ...
                    loss / log(2.0) / 4.7144, // echo "scale=4; $(wc -c < corpus.txt) / ($(wc -c < corpus.txt.bin) / 2)" | bc
